  ...
  <div class="mx-auto">
    <div class="leading-loose">
      ...
      <div class="px-10 py-5">
        <div class="">
          <label class="block text-sm text-gray-600" for="name">Laporan</label>
          <textarea
            class="w-full rounded bg-gray-200 px-5 py-2 text-gray-700 disabled:text-gray-500"
            rows="6"
            v-model="data.isiLaporan"
          ></textarea>
        </div>
        <div class="leading-loose">
          <div class="rounded bg-white p-10 shadow-xl">
            ...
            <div>
              <div v-for="({ filename, location }, i) in oldLampiran">
...
export default {
  ...
  methods: {
    ...
    async saveChange() {
      ...
      body.append("isiLaporan", this.data.isiLaporan);
      this.deleted.forEach((x) => body.append("deletedLampiran", x));
      this.$refs.lampiranFiles.forEach((x) =>
        body.append("lampiran", x.files[0])
      );
      const res = await this.$axios.$put(`pengaduan/${this.data.id}`, body);
      if (res.status) {
        Toast.fire({ icon: "success", title: "Success Update Pengaduan" });
